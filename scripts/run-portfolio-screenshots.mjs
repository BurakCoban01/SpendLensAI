...
const env = {
  ...process.env,
  PLAYWRIGHT_PORTFOLIO_SCREENSHOTS: "true",
  SPENDLENS_E2E_MEMORY_ADAPTERS: "false",
  SPENDLENS_E2E_API_PORT: process.env.SPENDLENS_E2E_API_PORT ?? "4103",
  SPENDLENS_E2E_WEB_PORT: process.env.SPENDLENS_E2E_WEB_PORT ?? "3003",
  OCR_SERVICE_URL: process.env.OCR_SERVICE_URL ?? "http://127.0.0.1:18622",
  SPENDLENS_E2E_NEXT_DIST_DIR: process.env.SPENDLENS_E2E_NEXT_DIST_DIR ?? ".next-e2e-portfolio",
  DATABASE_URL:
...
runWithRetries("pnpm", ["db:migrate"], env, 3);
run("pnpm", ["exec", "playwright", "test", "e2e/portfolio-screenshots.spec.ts"], env);
...
function runWithRetries(command, args, childEnv, attempts) {
