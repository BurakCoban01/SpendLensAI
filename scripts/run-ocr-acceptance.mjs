...
const env = {
  ...
  SPENDLENS_E2E_API_PORT: process.env.SPENDLENS_E2E_API_PORT ?? "4102",
  SPENDLENS_E2E_WEB_PORT: process.env.SPENDLENS_E2E_WEB_PORT ?? "3002",
  OCR_SERVICE_URL: process.env.OCR_SERVICE_URL ?? "http://127.0.0.1:18622",
  SPENDLENS_E2E_NEXT_DIST_DIR: process.env.SPENDLENS_E2E_NEXT_DIST_DIR ?? ".next-e2e-ocr",
  DATABASE_URL:
...
run("pnpm", ["exec", "playwright", "test", "e2e/ocr-upload-flow.spec.ts"], env);
